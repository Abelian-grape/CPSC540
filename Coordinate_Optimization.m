...
fp r in t f ( ’ P a s s e s = %d , f u n c ti o n = %.4e , change = %.4 f \n ’ , i /d , l o g i s t i c L 2 l o s s (w,X, y , lambda ) , change ) ;
i f change < progTol
...
The performance will vary based on the randomization/hardware, but on my laptop this decreased the
time from around 20 seconds down to around 4 seconds. The algorithm still takes 500 iterations and
finds a slightly lower function value of 1.473e + 02 in Matlab notation.
2. The code should look roughly like this (note that we now compute a vector of L values, use sampleDiscrete
to pick j, and use L(j) as the step-size).
function [ model ] = l o g i s t i c L 2 (X, y , lambda )
% Add b i a s v a r i a b l e
[ n , d ] = s i z e (X ) ;
X = [ one s ( n , 1 ) X ] ;
